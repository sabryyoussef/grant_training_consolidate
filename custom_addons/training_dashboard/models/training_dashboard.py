...
class TrainingDashboard(models.Model):
    ...
    def _check_grants_models(self):
        """Check if grants_training_suite_v19 models are available."""
        return (
            'gr.student' in self.env and
            'gr.course.integration' in self.env and
            'gr.progress.tracker' in self.env
        )
    ...
    def _compute_kpi_metrics(self):
        ...
        for dashboard in self:
            if not dashboard._check_grants_models():
                # Set default values if models not available
                dashboard.total_students = 0
                dashboard.enrolled_students = 0
                dashboard.completed_students = 0
                dashboard.completion_rate = 0.0
                dashboard.avg_completion_time = 0.0
                dashboard.active_courses = 0
            ...
            if completed_students:
                ...
                for student in completed_students:
                    if hasattr(student, 'intake_date') and student.intake_date and student.create_date:
    ...
    def _compute_progress_analytics(self):
        ...
        for dashboard in self:
            if not dashboard._check_grants_models():
                dashboard.progress_distribution = '{}'
                dashboard.monthly_enrollments = '{}'
                dashboard.completion_trends = '{}'
                continue
            ...
            for tracker in trackers:
                progress_range = dashboard._get_progress_range(tracker.overall_progress)
        ...
        for dashboard in self:
            ...
            for student in top_performers:
                top_data.append({
                    ...
                    'courses_completed': student.completed_courses if hasattr(student, 'completed_courses') else 0
                })
    ...
    def _compute_integration_analytics(self):
        ...
        for dashboard in self:
            if not dashboard._check_grants_models():
                dashboard.integration_status_summary = '{}'
